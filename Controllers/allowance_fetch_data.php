...
// SQL query to fetch data from salaryextras table (unchanged)
$sql = "SELECT SalaryExtras_id, Name, Description, Amount FROM salaryextras";
...
if ($result && $result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        echo '<tr>';
        echo '<td style="text-align: center;">' . $row["SalaryExtras_id"] . '</td>';
        ...
        // Edit Button
        echo '<button class="btn btn-primary edit-allowance-btn" title="Edit" data-toggle="modal" data-target="#editAllowanceModal"
                data-id="' . $row["SalaryExtras_id"] . '" 
        ...
        echo '<button class="btn btn-secondary delete-allowance-btn" title="Delete" data-toggle="modal" data-target="#deleteAllowanceModal"
                data-id="' . $row["SalaryExtras_id"] . '"
                data-name="' . htmlspecialchars($row["Name"]) . '"
